...
class ImagePredictor {
    ...
    static func createImageClassifier() -> VNCoreMLModel {
        ...
        let imageClassifierWrapper = try? BirdsClassifier(configuration: defaultConfig)
    ...
    func makePredictions(for photo: UIImage, completionHandler: @escaping ImagePredictionHandler) throws {
        let orientationValue = UInt32(photo.imageOrientation.rawValue)
        guard let orientation = CGImagePropertyOrientation(rawValue: orientationValue) else { return }
    ...
    private func visionRequestHandler(_ request: VNRequest, error: Error?) {
        ...
        predictions = observations.map { observation in
            ...
            Prediction(classification: observation.identifier,
                       confidencePercentage: observation.confidence.magnitude.description)
        }
    }
}
